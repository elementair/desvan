    ...
    class WordPressSettingsFramework {
    	...
    	public function generate_setting( $args ) {
    		...
    		switch( $type ){
    		    ...
    		    case 'file':
                        ...
                        $logo = get_options_page('logo_email');
                    if (empty($logo)) :
                        ?>
                        <img width="50" height="50" src="<?php echo plugins_url('suscriptflech/admin/assets/images/logo.png') ?>">
                    <?php else: ?>  
                    ...
                    jQuery(document).ready(function($){
                		$("#'. $el_id .'_button").click(function() {
                        	...
                        	window.send_to_editor = function(html) {
                                ...
                                if(!imgurl){imgurl = $("img", html).attr("src");}
                        		$("#'. $el_id .'").val(imgurl);
                        		tb_remove();
    		        ...
    		        echo '<input type="number" name="'. $this->option_group .'_settings['. $el_id .']" id="'. $el_id .'" value="'. $val .'" placeholder="'. $placeholder .'" min="'.$min.'" max="'.$max.'" class="'. $class .'" />';
        ...
        public function settings() {
            ...
            <form action="options.php" method="post">
        		...
        		<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Guardar cambios' ); ?>" /></p>
			</form>
    		<?php
        ...
        public function tab_styles() {
            ...
            <style type="text/css">
                ...
                .nav-tab-active, .nav-tab-active:focus, .nav-tab-active:focus:active, .nav-tab-active:hover
                {
                    ...
                    .postbox .dashicons-admin-generic {
                        margin-top: -3px;
                        margin-left: -23px;
                    }
...
if (!function_exists('get_options_page')) {

    function get_options_page($value) {
        $settings = wpsf_get_settings('suscriptflech_settings');

        if (isset($settings['general_' . $value]))
            return $settings['general_' . $value];
        if (isset($settings['recaptcha_' . $value]))
            return $settings['recaptcha_' . $value];
        if (isset($settings['mensajes_' . $value]))
            return $settings['mensajes_' . $value];
        if (isset($settings['plantilla_' . $value]))
            return $settings['plantilla_' . $value];
