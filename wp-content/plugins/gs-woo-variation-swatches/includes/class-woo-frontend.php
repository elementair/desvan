...
if ( ! class_exists( 'Gs_Woov_Frontend' ) ) {
	...
	class Gs_Woov_Frontend {
		...
		public function enqueue_scripts(){
			...
			if( isset( $data['attribute_style'] ) ){
				$attribute_style=$data['attribute_style'];
			}
			...
			wp_localize_script( 'gs-woo-frontend', 'gs_wccl_general', array(
				...
				'add_cart'      => apply_filters( 'gs_woov_add_to_cart_button_content', isset( $data['cart_button'] )? esc_html($data['cart_button']):esc_html__('add to cart','gs-variation' ) ),
				'grey_out'		=> $attribute_behavior,
				'attr_style'		=> (isset( $attribute_style )? $attribute_style:'square' ),
		...
		public function add_select_options( $html = '', $product = false ){
			...
			if(isset($data['enable_ajax_archive_page']) && $data['enable_ajax_archive_page']== '1'){
				...
				$data_product_variations =esc_attr( json_encode( $available_variations ) );
			}
		...
		public function add_to_cart_ajax_loop(){
			...
			parse_str( esc_attr( $_REQUEST['attr']), $attributes );
